defmodule Mix.Tasks.ExFingerprint.VerifySolverAlignment do
  use Mix.Task
  ...
  defp resolve_profile(profile_id) do
    {:ok, ExFingerprint.Profile.resolve(profile_id)}
  rescue
  ...
  defp solver_report(profile_id, opts) do
    ...
    with {:ok, result} <-
           ExFingerprint.Solver.Chrome.solve("https://tls.peet.ws/api/all", solver_opts),
