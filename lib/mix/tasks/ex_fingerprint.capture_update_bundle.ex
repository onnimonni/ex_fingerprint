defmodule Mix.Tasks.ExFingerprint.CaptureUpdateBundle do
  use Mix.Task
  ...
  def run(args) do
    ...
    with profile <- ExFingerprint.Profile.resolve(profile_id),
         {:ok, browser} <-
           ExFingerprint.Solver.Chrome.browser_info(browser_path: opts[:browser_path]),
         {:ok, client_hello} <-
           ExFingerprint.Capture.ClientHello.capture(
      ...
      output_dir =
        opts[:output_dir] ||
          Application.app_dir(:ex_fingerprint, "priv/captures/chrome#{browser.major}")
  ...
  defp fast_report(profile_id) do
    request =
      ExFingerprint.Request.new("https://tls.peet.ws/api/all", browser_profile: profile_id)

    with {:ok, response} <- ExFingerprint.request(request, solver: false),
  ...
  defp solver_report(profile_id, opts) do
    with {:ok, result} <-
           ExFingerprint.Solver.Chrome.solve(
