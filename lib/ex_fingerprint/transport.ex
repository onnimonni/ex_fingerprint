defmodule ExFingerprint.Transport do
  @moduledoc """
  Main request orchestration.
  """

  alias ExFingerprint.AntiBot
  alias ExFingerprint.Client
  alias ExFingerprint.CookieJar
  alias ExFingerprint.Request
  alias ExFingerprint.Response
  ...
  def request(%Request{} = request, opts \\ []) do
    solver_backend = Keyword.get(opts, :solver_backend, ExFingerprint.Solver.Chrome)
    solver_registry = Keyword.get(opts, :solver_registry, ExFingerprint.Solver.Registry)
    solver_enabled? = Keyword.get(opts, :solver, true)
    rotator = Keyword.get(opts, :rotator, ExFingerprint.Rotator)
  ...
  defp solve_and_retry(response, request, solver_backend, solver_registry, details, rotator, opts) do
    cookie_jar = Keyword.get(opts, :cookie_jar, ExFingerprint.CookieJar)
    ...
    if ExFingerprint.Solver.Registry.circuit_open?(solver_registry, request, solver_opts) do
      {:ok,
      ...
      with {:ok, solve_result} <-
             ExFingerprint.Solver.Registry.lookup_or_solve(solver_registry, request, solver_opts),
  ...
  defp handle_solver_retry_result(
    ...
    case AntiBot.classify(retried_response) do
      {:ok, details} ->
        :ok =
          ExFingerprint.Solver.Registry.mark_retry_succeeded(
            solver_registry,
            request,
            solver_opts
          )
      ...
      {:challenge, retry_details} ->
        circuit =
          ExFingerprint.Solver.Registry.mark_retry_failed(solver_registry, request, solver_opts)
