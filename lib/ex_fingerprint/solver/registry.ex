defmodule ExFingerprint.Solver.Registry do
  @moduledoc """
  ...
  defp start_job(%__MODULE__{} = state, key, %Request{} = request, opts, waiters) do
    task =
      Task.Supervisor.async_nolink(ExFingerprint.Solver.TaskSupervisor, fn ->
  ...
  defp request_key(%Request{} = request, opts) do
    backend =
      opts
      |> Keyword.get(:backend, ExFingerprint.Solver.Chrome)
      |> inspect()
