defmodule ExFingerprint.RequestIntegrationTest do
  use ExUnit.Case, async: false

  alias ExFingerprint.Request
  alias ExFingerprint.Rotator
  alias ExFingerprint.Solver.Registry
  alias ExFingerprint.TestSupport.CountingSolver
  alias ExFingerprint.TestSupport.HTTPServer
  alias ExFingerprint.TestSupport.TestSolver

  setup do
    :ok = ExFingerprint.clear_cookies()
  ...
  test "executes a plain http request through the native transport" do
    ...
    assert {:ok, response} =
             ...
             |> ExFingerprint.request(solver: false)
  ...
  test "can disable redirect following while still storing redirect cookies" do
    ...
    assert {:ok, response} =
             ExFingerprint.request(HTTPServer.url(port, "/start"),
               solver: false,
    ...
    assert {:ok, %{status: 200, body: "cookie-kept"}} =
             ExFingerprint.request(HTTPServer.url(port, "/final"),
               solver: false,
               metadata: bucket
             )
  end
  ...
  test "marks challenge responses as unsolved when the solver fails" do
    ...
    assert {:ok, response} =
             ...
             |> ExFingerprint.request(
               solver: true,
               solver_backend: ExFingerprint.RequestIntegrationTest.FailingSolver,
               rotator: :test_rotator
             )
  ...
  test "opens the solver circuit after repeated challenged retries and skips further launches" do
    ...
    assert {:ok, first_response} =
             ExFingerprint.request(HTTPServer.url(port), solver_opts)
  ...
  test "marks solver overload when the browser queue is saturated" do
    ...
    running_task =
      Task.async(fn ->
        ExFingerprint.request(HTTPServer.url(port),
          solver: true,
          solver_backend: CountingSolver,
    ...
    assert {:ok, overloaded_response} =
             ExFingerprint.request(overloaded_url,
               solver: true,
               solver_backend: CountingSolver,
               solver_registry: :overloaded_solver_registry,
               notify_pid: test_pid
  ...
  test "notifies the rotator for rate limited responses" do
    ...
    assert {:ok, response} =
             HTTPServer.url(port)
             |> Request.new(browser_profile: :chrome_latest)
             |> ExFingerprint.request(solver: true, rotator: :test_rotator)
  ...
  test "shares cookies for the same wireguard bucket and keeps them isolated across buckets" do
    ...
    assert {:ok, %{status: 200}} =
             ExFingerprint.request(HTTPServer.url(port, "/set"),
               solver: false,
               metadata: bucket_a
             )

    assert {:ok, %{status: 200, body: "shared-ok"}} =
             ExFingerprint.request(HTTPServer.url(port, "/use"),
               solver: false,
               metadata: bucket_a
             )
    ...
    assert :ok = ExFingerprint.clear_cookies(wireguard_ip_address: "10.0.0.2")
  ...
  test "can clear cookies by request scope even when the wireguard bucket stays the same" do
    ...
    assert {:ok, %{status: 200, body: "scoped-ok"}} =
             ExFingerprint.request(request, solver: false)

    assert :ok = ExFingerprint.clear_cookies(request)
  ...
  test "returns an error when redirects exceed the configured limit" do
    ...
    assert {:error, :too_many_redirects} =
             ExFingerprint.request(HTTPServer.url(port, "/loop"),
               solver: false,
               max_redirects: 0
             )
  end
...
defmodule ExFingerprint.RequestIntegrationTest.FailingSolver do
