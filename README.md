...
- `ex_fingerprint` uses kernel WireGuard managed from Elixir through `wireguardex`
...
- profile maintenance tasks:
  `mix ex_fingerprint.export_profile`
  `mix ex_fingerprint.import_profile`
  `mix ex_fingerprint.set_latest`
- custom CA bundle install path:
  `mix ex_fingerprint.install_ca_bundle path/to/roots.pem`
- real Chrome CDP solver path for cookie extraction after challenge escalation
- real Chrome update/capture tasks:
  `mix ex_fingerprint.capture_tls_clienthello`
  `mix ex_fingerprint.verify_solver_alignment`
  `mix ex_fingerprint.capture_update_bundle`
...
`ExFingerprint.Solver.Chrome` now launches real Chrome over CDP, opens a page,
...
1. `mix ex_fingerprint.capture_update_bundle`
...
5. `mix ex_fingerprint.set_latest chrome_NNN`
6. rerun:
   `mix ex_fingerprint.verify_peet`
   `mix ex_fingerprint.verify_solver_alignment`
